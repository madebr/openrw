...
struct PyMessageReceiver : public Logger::MessageReceiver {
    using Logger::MessageReceiver::MessageReceiver;
...
void register_logging(py::module& m) {
    py::module logging = m.def_submodule("logging", "Logging submodule");

//    py::class_<Logger::MessageReceiver, PyMessageReceiver> messageReceiver(logging, "MessageReceiver");
    py::class_<Logger::MessageReceiver, PyMessageReceiver, std::shared_ptr<Logger::MessageReceiver>> messageReceiver(logging, "MessageReceiver");
//    py::class_<PyMessageReceiver, Logger::MessageReceiver, std::shared_ptr<PyMessageReceiver>> messageReceiver(logging, "MessageReceiver");
    messageReceiver
            .def(py::init<>())
